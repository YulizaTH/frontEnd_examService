...
const EXAM_SERVICE = new Vuex.Store({
    ...
    actions: {
        loadExams({commit}, {self}) {
            Axios.get(ENV.API + "/exam/listExamUser/" + VueLocalStorage.get("auth").id)
                .then((r) => {
                    if (r.status === 200) {
                        self.data = r.data;
        ...
        loadExam({commit}, {self}){
            Axios.get(ENV.API + "/exam/takeExam/"+ self.theme_id)
                .then((r) => {
                    if (r.status === 200) self.data = r.data
                })
                .catch((e) => Util.fnError(e) )
        },

        loadExamsSolution({commit}, {self}){
            Axios.get(ENV.API + "/exam/seeExamSolution/1")
                .then((r) => {
